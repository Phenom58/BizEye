...
export default function Landing({ onGetStarted, isLoggedIn, onGoToDashboard, onLogout }: Props) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white overflow-x-hidden">
      ...
      {/* Hero Section */}
      <section className="relative z-10 px-8 md:px-16 pt-20 pb-32 min-h-[calc(100vh-80px)] flex items-center">
        <div className="max-w-5xl">
          ...
          <h1 className="text-5xl md:text-7xl lg:text-8xl font-bold leading-[1.05] mb-8 animate-slide-up delay-100">
          ...
          <p className="text-gray-400 text-lg md:text-xl max-w-xl leading-relaxed mb-12 animate-slide-up delay-200">
          ...
          <div className="flex flex-wrap items-center gap-4 animate-slide-up delay-300">
            ...
            <a href="#features" className="flex items-center gap-3 text-white/70 hover:text-white transition-colors px-4 py-4 cursor-pointer">
        ...
        {/* Floating Rotating Scroll indicator */}
      ...
      {/* Stats bar */}
      <section className="relative z-10 border-t border-white/[0.06] border-b bg-white/[0.01] backdrop-blur-xs">
        <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-white/[0.06]">
          <CountUpStat value="95%" label="Upload Success Rate" />
          <CountUpStat value="<10s" label="Dashboard Generation" />
          <CountUpStat value="85%+" label="Forecast Accuracy" />
          <CountUpStat value="3" label="Intelligence Pillars" />
        </div>
      </section>
          ...
          <div className="grid md:grid-cols-3 gap-px bg-white/[0.06]">
            {FEATURES.map((f, i) => (
              <div
                key={f.title}
                onClick={handlePrimaryAction}
                className="bg-[#0a0a0a] p-10 group hover:bg-[#111] transition-colors duration-300 cursor-pointer"
              >
      ...
      <section className="relative z-10 px-8 md:px-16 py-32 border-t border-white/[0.06]">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <button
            onClick={handlePrimaryAction}
            className="group inline-flex items-center gap-3 bg-sky-400 text-black font-semibold px-8 py-4 rounded-full hover:bg-sky-300 transition-all duration-300 cursor-pointer shadow-lg shadow-sky-400/20"
          >
